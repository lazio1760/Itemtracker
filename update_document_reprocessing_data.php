	...
	public function update_document_processing_status($internal_number,$receiptDate,$current_time,$action,$group,$documentType,$modified,$current_date,$fdaAppNumber){
		...
		if($group == '510K'){
			...
			if($action == "start"){
					...
					try{
					$find_doc_results = $hrtsConnection->prepare("SELECT * FROM `510k_processing` WHERE id = (select min(id) from `510k_processing` where internal_number = :internal_number AND date_received = :receiptDate)");
					...
					$find_doc_results->execute();
						...
						fclose($error_handle);
						die("<script type=\"text/javascript\"> alert('Check log for document reprocessing internal number query error.'); </script>");
					...
					$original_processor = $row["original_processor"];
				
				
					try{
						$results = $hrtsConnection->prepare("INSERT INTO `510k_processing` (status,start_process,document_type,modified_by,original_processor,internal_number,date_received,start_date,reprocessed,fda_application_number) VALUES ('processing','$current_time',:documentType,:modified,:original_processor,:internal_number,:receiptDate,'$current_date','yes',:fda_app_number)");
				
						$results->bindValue(':documentType', $documentType, PDO::PARAM_STR);
						$results->bindValue(':original_processor', $original_processor, PDO::PARAM_STR);
		...
		if($group == 'IDE'){
			...
			if($action == "start"){
				...
				try{
					$find_doc_results = $hrtsConnection->prepare("SELECT * FROM `ide_processing` WHERE id = (select min(id) from `ide_processing` where internal_number = :internal_number AND date_received = :receiptDate)");
				...
				try{
				$results = $hrtsConnection->prepare("INSERT INTO `ide_processing` (status,start_process,document_type,modified_by,original_processor,internal_number,date_received,start_date,reprocessed,fda_application_number) VALUES ('processing','$current_time',:documentType,:modified,:original_processor,:internal_number,:receiptDate,'$current_date','yes',:fda_app_number)");
		...
		if($group == 'PMA'){
			if($action == "start"){
				...
				try{
					$find_doc_results = $hrtsConnection->prepare("SELECT * FROM `pma_processing` WHERE id = (select min(id) from `pma_processing` where internal_number = :internal_number AND date_received = :receiptDate)");
				...
				try{
					$results = $hrtsConnection->prepare("INSERT INTO `pma_processing` (status,start_process,document_type,modified_by,original_processor,internal_number,date_received,start_date,reprocessed,fda_application_number) VALUES ('processing','$current_time',:documentType,:modified,:original_processor,:internal_number,:receiptDate,'$current_date','yes',:fda_app_number)");
		...
		if($group == 'RAD_HEALTH'){
			if($action == "start"){
					...
					try{
					$find_doc_results = $hrtsConnection->prepare("SELECT * FROM `rad_processing` WHERE id = (select min(id) from `rad_processing` where internal_number = :internal_number AND date_received = :receiptDate)");
				...
				try{
				$results = $hrtsConnection->prepare("INSERT INTO `rad_processing` (status,start_process,document_type,modified_by,original_processor,internal_number,date_received,start_date,reprocessed,fda_application_number) VALUES ('processing','$current_time',:documentType,:modified,:original_processor,:internal_number,:receiptDate,'$current_date','yes',:fda_app_number)");
		...
		if($group == '2579'){
			if($action == "start"){
				try{
				$find_doc_results = $hrtsConnection->prepare("SELECT * FROM `2579_processing` WHERE id = (select min(id) from `2579_processing` where internal_number = :internal_number AND date_received = :receiptDate)");
				...
				try{
				$results = $hrtsConnection->prepare("INSERT INTO `2579_processing` (status,start_process,document_type,modified_by,original_processor,internal_number,date_received,start_date,reprocessed) VALUES ('processing','$current_time',:documentType,:modified,:original_processor,:internal_number,:receiptDate,'$current_date','yes')");
		...
		if($group == '513G'){
			if($action == "start"){
				if(!(empty($fdaAppNumber))){//die($fdaAppNumber);
					try{
					$find_doc_results = $hrtsConnection->prepare("SELECT * FROM `513g_processing` WHERE id = (select min(id) from `513g_processing` where fda_application_number = :fda_app_number)");
					...
					try{
					$find_doc_results = $hrtsConnection->prepare("SELECT * FROM `513g_processing` WHERE id = (select min(id) from `513g_processing` where internal_number = :internal_number AND date_received = :receiptDate)");
